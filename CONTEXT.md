...
**Last updated:** 2026-09-24 · **Branch:** `main` (Phase 12e follow-up merged 2026-09-24 via `phase-12e/practice-live-history-responsive`, `--no-ff`; no new tag — follow-up on `phase-12-complete` + 12b/12c/12d; `v0.2.0-pilot` intentionally waits for Phase 11) · **Remote:** `origin` = `git@github.com:kspranav-az/ZIOS.git` (SSH; all branches + tags pushed)
...
- Orchestrator: 121 passed / 2 skipped, ruff + mypy strict clean (`uv run pytest` etc. in `services/ai-orchestrator`)
...
1. **By-hand Gemini-mode smoke of the live practice room** (Phase 12e gate (d), not yet done): with the host orchestrator up (`.env.host`, `--reload`) and `LLM_MODE=gemini`, run one real live practice session through the Ascend UI — room connects, conductor speaks, transcript lands, report generates.
2. **Phase 11 kickoff:** notifications (WhatsApp/SMS), Razorpay behind flag (credit top-ups for both products), pilot hardening + load test; tag `v0.2.0-pilot` after.
3. Owner review of `docs/partner-integration-runbook.md` + `docs/ascend-beta-runbook.md` + `docs/PRESENTATION.md`.
4. Live-capture proof on a real voice/AI-video session (see §9).
5. Ascend beta dry-run with a closed user cohort (readiness-formula feedback before the voice drop).
6. Provider procurement (LLM, TTS, Google OAuth, WhatsApp, payments).
7. Pilot prep: ≥3 pilot employers (PRD X9).
8. Fix the pre-existing `jd-generation.spec.ts` expectation (topic-grouped regenerate buttons) next time generation code is touched.
