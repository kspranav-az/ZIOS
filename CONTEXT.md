...
1. **By-hand Gemini-mode smoke of the live practice room, take two** (Piper TTS now live on the host orchestrator): the conductor should speak intelligible speech instead of the mock tone; transcript will still ride mock STT until the room-STT factory phase.
2. **Wire the voice room STT to the adapter factory** (room sessions still construct `MockSttAdapter()` directly; streaming GCP STT for rooms is its own phase — see §5 Phase-12f gotchas).
