#!/usr/bin/env node
/**
 * Seed an AI-led text interview with structured answers for manual validation.
 *
 *   node scripts/seed-structured-answers-validation.js
 *
 * Pre-requisites:
 ...
 *   - one text-mode kit with open_ended, mcq_single, mcq_multi, and rating_scale questions
...
async function main() {
  ...
  const candidateName = 'Structured Answers Candidate';

  console.log('1. Creating admin...');
  const { token: adminToken } = await signupAdmin(adminEmail);
  console.log(`   admin: ${adminEmail}`);

  console.log('2. Creating text-mode kit with structured questions...');
  const { kit } = await postJson(
    '/kits',
    {
      title: 'Structured Answers Validation Kit',
      role: 'Software Engineer',
      level: 'Mid',
      settings: { mode: 'text', proctoringLevel: 'none' },
    },
    { authorization: `Bearer ${adminToken}` },
  );

  const mcqSingleCorrect = randomUUID();
  const mcqMultiCorrectA = randomUUID();
  const mcqMultiCorrectB = randomUUID();

  const questions = [
    {
      type: 'open_ended',
      prompt: 'Tell us about a challenging project you led.',
      topic: 'Experience',
      timeLimitSec: 120,
      timeLimitType: 'soft',
      mandatory: true,
    ...
    },
    {
      ...
      followupPolicy: 'none',
      options: [
        { id: mcqSingleCorrect, text: 'PUT', correct: true },
        { id: randomUUID(), text: 'POST', correct: false },
        { id: randomUUID(), text: 'PATCH', correct: false },
      ],
      rubricLines: [{ id: randomUUID(), text: 'Accuracy', weight: 1 }],
    },
    {
      type: 'mcq_multi',
      prompt: 'Select all that are JavaScript primitive types.',
      topic: 'Technical',
      timeLimitSec: 60,
      timeLimitType: 'soft',
      mandatory: true,
      followupPolicy: 'none',
      options: [
        { id: mcqMultiCorrectA, text: 'string', correct: true },
        { id: mcqMultiCorrectB, text: 'number', correct: true },
        { id: randomUUID(), text: 'array', correct: false },
        { id: randomUUID(), text: 'object', correct: false },
      ],
      rubricLines: [{ id: randomUUID(), text: 'Accuracy', weight: 1 }],
    },
    {
      type: 'rating_scale',
      prompt: 'Rate your confidence with React hooks (1 = beginner, 5 = expert).',
  ...
  console.log('\n✅ Structured-answers interview seeded.\n');
  ...
  console.log('  b. Answer the open-ended question with any text.');
  console.log('  c. Select PUT for the MCQ single question.');
  console.log('  d. Select string and number for the MCQ multi question.');
  console.log('  e. Choose a rating for the rating_scale question.');
  console.log('  f. Complete the interview and wait for the report to generate.');
  console.log('  g. Open the employer report and confirm:');
  console.log('       - MCQ single shows "PUT" and scores 5.');
  console.log('       - MCQ multi shows "string, number" and scores 5.');
  console.log('       - Rating shows the chosen value as the score.');
  console.log('       - Evidence spans quote the selected options/rating.');
  console.log('');
