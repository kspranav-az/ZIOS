...
| -------------------------- | -------------------------------------------------- | ---------------------------------------------- |
| API unit + integration     | ✅ 364 passed / 2 skipped (66 files)               | `DATABASE_URL=postgresql://interviewos:interviewos_dev@localhost:55432/interviewos pnpm --filter @zios/api test` |
| Orchestrator pytest        | ✅ 118 passed / 2 skipped                          | `cd services/ai-orchestrator && uv run pytest` |
...
| Ascend unit                | ✅ 34 passed                                       | `pnpm --filter ascend-web test`                |
...
| Ascend E2E                 | ✅ 4 passed                                        | `pnpm --filter ascend-web e2e` (rebuild image first) |
...
| ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| Live LiveKit capture — validation gate (not build work) | Video-mode feature is complete and e2e-tested locally (candidate side included: VideoInterviewPage with adaptiveStream/dynacast, 5/5 candidate e2e). Unproven piece is real-network media transport: no real browser session over remote SFU/TURN has verified a recording lands in MinIO and analysis completes. Expected fix surface at validation: backend env config only (LIVEKIT_URL/keys/TURN/API_BASE_URL); candidate-side changes not anticipated — possible small patch only if real-device quirks surface (Safari/iOS codec negotiation → publish-defaults tweak) | Phase 11 validation run with pass criteria (see §6); keep row open until the run passes — validation may surface a small code patch, by design |
