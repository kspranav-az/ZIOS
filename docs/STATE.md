...
**Snapshot date:** 2026-09-24 · **HEAD:** `main` (Phase 12f follow-up merged via `phase-12f/piper-local-tts` + `--no-ff`, history preserved; no new tag — follow-up on `phase-12-complete` + 12b/12c/12d/12e; `v0.2.0-pilot` intentionally NOT tagged — that waits for Phase 11's pilot gate) · **Remote:** `origin` = `git@github.com:kspranav-az/ZIOS.git` (all branches + tags pushed) · **Tags:** `phase-00-complete` … `phase-09b-complete`, `phase-10-complete`, `phase-12-complete`, `phase-14-complete`, `v0.1.0-mvp0-mock` · **Phase 12f ✅ complete** (Piper local TTS behind `TTS_ADAPTER`; evidence below and in `phases/phase-12f-piper-local-tts.md`) — next: Phase 11, the room-STT factory wiring, then the by-hand live-practice smoke take two
...
| Orchestrator pytest        | ✅ 131 passed / 2 skipped                          | `cd services/ai-orchestrator && uv run pytest` |
...
### Post-M1: Phase 12f — Piper local TTS (✅ complete, merged 2026-09-24)

Follow-up on 12e (no new tag). Spec: `phases/phase-12f-piper-local-tts.md`. TTS moves from mock-only-by-code to `TTS_ADAPTER=mock|piper` behind a boot-time factory (mock stays the CI/e2e default; unknown values fail loudly before any request).

- `app/voice/piper_tts.py` — Piper (VITS via ONNX Runtime, espeak-ng phonemization, CPU-only, no API key) implementing `TtsPort`: sha256-pinned MIT `en_US-lessac-medium` voice, sentence buffering identical to the mock adapter, synthesis off the event loop via `asyncio.to_thread`, resample 22050→24000 Hz with soxr at the adapter boundary so the WS wire contract and all frontend playback stay untouched. `app/voice/tts_factory.py` mirrors the `STT_ADAPTER` factory pattern; the voice router builds the adapter once at import.
- Dockerfile: `espeak-ng` + the two model files baked sha256-verified into `/opt/piper-models` (`PIPER_MODEL_PATH`); `piper-tts` arrives via the frozen uv sync.
- Tests: 8 new (contract mirroring the mock suite, PCM16 non-silence at the wire rate, punctuation-less tail semantics pinned, healthcheck shape, factory matrix incl. missing-model fail-loud); module skips without the downloaded model so CI stays hermetic. Suite **131 passed / 2 skipped**, ruff + mypy strict clean.
- Validation: host probe `chunks=7 samples=102539 peak=32767 seconds=4.27`; linux/amd64 image parity probe green (`status: healthy, provider: piper` inside the built image). `.env.host` (untracked) sets `TTS_ADAPTER=piper`; compose/CI default stays mock.
- ~~**Known gap recorded:** the voice room STT is still `MockSttAdapter()` constructed directly in `app/voice/router.py` — `STT_ADAPTER` applies only to the analysis pipeline.~~ **Resolved 2026-09-24** (branch `feat/live-room-gcp-stt`): the room STT is factory-driven like TTS, the frontend streams real 16 kHz mic audio (`packages/interview-room/mic-capture.ts`), and the socket is multi-turn (`awaiting_answer` / `interview_complete`). The GCP adapter's batch `transcribe_stream` (one final per turn, no partials, 60 s internal chunking) serves the room; a true streaming GCP adapter remains a procurement-era nicety.
...
| TTS                                | Orchestrator contract → `MockTtsAdapter` / `PiperTtsAdapter` (`TTS_ADAPTER=mock\|piper`)                                     | Deterministic silence fixtures; Piper validated 2026-09-24 (local CPU neural voice, MIT model)                                                                 | Prompt tuning + cost validation against real traffic |
...
| Phase 12f shipped (2026-09-24)                 | —                                                                                                                                                 | ✅ Piper local TTS behind `TTS_ADAPTER` (factory + adapter + image bake + parity probe; wire contract unchanged). No new tag. Evidence in `phases/phase-12f-piper-local-tts.md` |
| Voice-room STT ignores `STT_ADAPTER`           | ~~Live rooms transcribe with the fixture mock regardless of env — real speech becomes fixture text (seen in the 12e smoke); only the analysis pipeline uses the STT factory~~ **Fixed 2026-09-24** (`feat/live-room-gcp-stt`): room STT factory-driven, real 16 kHz mic streaming from the frontend, multi-turn socket | Remaining nicety only: a *streaming* GCP adapter for mid-speech partials (batch adapter serves rooms fine) |
| Phase 12e gate (d) — by-hand gemini live-practice smoke | E2E proves the live room hermetically in mock mode; a human ran one session (telemetry-serialization bug found + fixed) but has not re-run since Piper TTS went live | Host orchestrator up (`.env.host` sets `TTS_ADAPTER=piper`) + `LLM_MODE=gemini`: run one live practice session — expect intelligible conductor speech; transcript still mock-STT until the row above closes |
...
- **Phase 12f (2026-09-24):** `phase-12f/piper-local-tts` → `main` via `--no-ff` merge (no new tag; follow-up on `phase-12-complete` + 12b/12c/12d/12e per the phase file). Branch preserved and pushed.
...
1. **By-hand live-practice smoke, take three:** host orchestrator up (`.env.host` sets `TTS_ADAPTER=piper`, `STT_ADAPTER=gcp` + `GCP_PROJECT_ID`, ADC auto-discovered), `LLM_MODE=gemini` — one live practice session through the Ascend UI; expect intelligible Piper speech, turn-end captions (GCP STT has no partials), a full multi-turn interview, and `interview_complete` → report.
2. ~~**Wire the voice-room STT to the adapter factory**~~ **(done 2026-09-24, `feat/live-room-gcp-stt`):** room STT is factory-driven (`STT_ADAPTER`), real mic audio streams from all three room pages, and the socket is multi-turn with `awaiting_answer`/`interview_complete`.
3. **Phase 11 kickoff:** pilot hardening, notifications (WhatsApp/SMS), Razorpay behind flag (serves both Meridian and Ascend credit top-ups), load test — milestone tag `v0.2.0-pilot` comes after Phase 11, not before.
4. **Owner review:** read through `docs/partner-integration-runbook.md` (partner-facing), `docs/PRESENTATION.md` (demo script), and `docs/ascend-beta-runbook.md` (Ascend beta ops) — the last open validation items.
5. **Live capture proof (Phase 11 validation gate — blocked on reachable LiveKit, not on code):** point compose at a reachable LiveKit (Cloud or self-hosted + coturn) via env (`LIVEKIT_URL`, key/secret, `API_BASE_URL` callback) and run:
...
6. **Provider procurement:** Select and obtain credentials for LLM, STT, TTS, Google OAuth, WhatsApp, and payments.
7. **Ascend beta dry-run with real users:** closed cohort on the grant-funded beta; collect readiness-formula feedback (credibility of D14/D15) before the voice drop. The live-practice mode (12e) is the headline feature for this cohort.
8. **Pilot preparation:** Identify ≥ 3 pilot employers per PRD exit criterion X9.
9. **Fix the pre-existing `jd-generation.spec.ts` expectation** (11 topic-grouped regenerate buttons vs 12 asserted) next time generation code is touched.
