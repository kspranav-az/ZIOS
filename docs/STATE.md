...
- **Known gap recorded:** the voice room STT is still `MockSttAdapter()` constructed directly in `app/voice/router.py` — `STT_ADAPTER` applies only to the analysis pipeline. Wiring room STT to the factory (incl. streaming GCP STT for rooms) is its own phase.
...
| Voice-room STT ignores `STT_ADAPTER`           | Live rooms transcribe with the fixture mock regardless of env — real speech becomes fixture text (seen in the 12e smoke); only the analysis pipeline uses the STT factory | Dedicated phase: wire room STT through a factory (`STT_ADAPTER=mock\|gcp` for rooms incl. streaming GCP STT, which needs a streaming adapter beyond today's batch GCP adapter) |
...
1. **By-hand live-practice smoke, take two:** host orchestrator up (`.env.host` now sets `TTS_ADAPTER=piper`), `LLM_MODE=gemini` — one live practice session through the Ascend UI; expect intelligible conductor speech (Piper) and gemini judging; transcript still rides mock STT until item 2.
2. **Wire the voice-room STT to the adapter factory** (rooms construct `MockSttAdapter()` directly today; streaming GCP STT for rooms is a separate adapter beyond the batch one).
