...
**Companion document: *AI Interview Ecosystem — Product & Engineering Blueprint v1.0* (architecture depth lives there; this PRD governs scope)**
...
- *A1 — Validation layer meaning:* the existing product pushes candidates (with a JD or role context) to this platform via API; the platform conducts/hosts the interview and returns structured results (scorecard + evidence) via webhook. The MVP also validates market demand for the ecosystem itself.
- *A2 — Team & window:* 2–4 engineers, 8–10 weeks to pilot-ready. Scope is cut to fit; §15 marks what moves if the team is smaller.
- *A3 — Launch market:* India-first; DPDP Act compliance baseline; English UI with Hindi/Hinglish-tolerant voice interviews.
...
**Within 10 minutes of signing up, an employer can turn a JD (or a manual configuration) into a published, structured interview; send candidates a link; and — after the candidate completes a text, voice, or video interview conducted by AI (or facilitated by a human interviewer, with baseline proctoring) — receive, within 5 minutes, an evidence-linked evaluation report they can act on.**
...
| E6 Candidate experience | Mobile-first web (no app install); preflight checks (mic/camera/network); AI-use + recording consent flow; practice question; all three modes; session recovery on network drop; completion confirmation |
...
| FR-E10-5 | Candidate comparison view (per kit): side-by-side scores + evidence | P1 | Compare ≥ 2 candidates |
...
| | **Text (AI)** | **Voice (AI)** | **Video (AI)** | **Human-facilitated** |
|---|---|---|---|---|
| Channel | Chat UI | WebRTC audio | WebRTC audio+camera | Live video room |
| Conductor | AI | AI | AI | Human (kit-guided) |
| Follow-ups | Fixed or adaptive | Fixed or adaptive | Fixed or adaptive | Human judgment (+ kit suggestions) |
| Recording | Transcript | Audio + transcript | AV + snapshots + transcript | AV + transcript |
| Proctoring signals | Tab/fullscreen events, paste events | Audio events, silence markers | Snapshots, all voice signals | Human observation + recording |
| Best for | Low bandwidth, screening at scale, MCQ-heavy | Default India-first depth screen | Higher-stakes or client-facing roles | Final rounds, senior roles |
| COGS/class | 1 credit | 2 credits | 3 credits | 1 credit |

Mode is chosen **per kit** (all questions in one mode for comparability in M1; mixed-mode kits arrive in M2). Video-ON is **off by default** — employer consciously enables it (candidate bandwidth + comfort; AI-conducted video adds cost without scoring value in M1, since we do no face-based analysis — and never will, per the Blueprint's no-emotion-recognition policy [^48^]).
...
Every transition emits an analytics event and (for subscribed orgs) a webhook. `Abandoned → Invited` triggers the nudge sequence — show-rate is a core employer metric (§12).
...
| W8 | Human-facilitated room, cockpit, scorecard, auto-notes | Employer dashboard polish, PDF export | Human-facilitated interview end-to-end |
| W9 | Integration API + webhooks + sandbox; credits wallet; cost/latency tuning; red-team pass | Candidate flow polish; comparison view | X-criteria dry-run on staging data |
...
1. **Pilot pricing:** free credits then paid, or paid from day one? *Recommend:* 25 free credits/org during pilot, paid packs immediately after — willingness-to-pay data starts early.
2. **Candidate OTP on by default?** *Recommend:* employer-optional, default off for friction, default on for strict proctoring kits.
3. **External question-API partner:** build the adapter against a real partner now or stub? *Recommend:* adapter interface + stub in M1; real partner when a pilot demands it.
...
*Internal note: section references of the form "Blueprint §x.y" point to the companion document, AI Interview Ecosystem — Product & Engineering Blueprint v1.0.*
