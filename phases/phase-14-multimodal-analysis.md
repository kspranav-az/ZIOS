...
- [x] Video-mode candidate track captured to MinIO and analyzable; voice-mode recordings analyzable (audio features) — **signed off at unit/integration level** (owner decision 2026-09-12): `tests/test_video_capture.py` (16 tests: ffprobe-verified webm encode, audio-only fallback, `media_kind` routing) and API processor audio-kind spec pass. First live LiveKit capture proof is expected on the next real voice/AI-video session and tracked as a known gap in `docs/STATE.md`.
