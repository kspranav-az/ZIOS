...
Stand up a repo, toolchain, CI, and Docker-first dev environment that every later phase builds on — with zero product features. Quality gates must exist *before* feature code so nothing merges unguarded.
...
- [ ] `docker compose up -d` → all services healthy (postgres, redis, minio, minio-init done, mailpit, livekit, api, ai-orchestrator)
- [ ] CI green: lint, typecheck, unit tests, image builds, integration smoke tests
- [ ] Branch protection + commitlint reject non-conventional commit messages
- [ ] Migrations run forward and rollback cleanly (expand-migrate-contract verified)
- [ ] Skeleton services log structured JSON with correlation IDs
- [ ] ADRs 0001–0002 merged; module-boundary lint rule demonstrably fails on a cross-context import

## Validation ✔️

- [ ] New-clone bootstrap ≤ 15 min following README alone (timed on a clean machine/VM)
- [ ] No secrets or provider keys anywhere in the repo (scan passes)
- [ ] WhatsApp template-approval and Razorpay KYC applications submitted (ticket/tracking ref recorded in `docs/`)
- [ ] Team can explain the ports-and-adapters rule and where each future provider plugs in
