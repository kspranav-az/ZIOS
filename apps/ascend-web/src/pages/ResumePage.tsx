  ...
  const handleFile = async (file: File) => {
    ...
    if (isPdf) {
      ...
      try {
        ...
        setError(
          err instanceof ApiErrorResponse ? err.message : 'PDF upload failed. Please try again.',
        );
      } finally {
        setUploading(false);
      }
    ...
    if (!text.trim() || text.includes('')) {
      setError(
        'This file type is not supported yet — upload a .txt or .pdf resume, or paste the plain text below.',
      );
      return;
    }
    setResumeText(text);
  ...
  return (
    <div className="mx-auto w-full max-w-3xl space-y-6">
      ...
      <Card padding="lg" radius="2xl">
        ...
        <label
          htmlFor="resume-text"
          className="mt-4 block text-label-bold uppercase tracking-wide text-on-surface-variant"
        >
      ...
      {parsed && (
        <Card padding="lg" radius="2xl">
          ...
          <p className="mt-1 text-body-sm text-on-surface-variant">
            Paste a job description to see keyword coverage, gaps, and honest rewrite suggestions —
            fabricated metrics are flagged, never presented as yours.
          </p>
                      ...
                      <div key={index} className="rounded-xl bg-surface-container-low p-4">
                        ...
                        {suggestion.honestyFlags.length > 0 && (
                          <p className="mt-2 flex items-start gap-1 rounded-lg bg-error-container p-2 text-body-sm text-on-error-container">
                            ...
                            This rewrite contains numbers not in your resume — verify before using:{' '}
                            {suggestion.honestyFlags.join(', ')}
                          </p>
                        )}
                      </div>
                    ))}
